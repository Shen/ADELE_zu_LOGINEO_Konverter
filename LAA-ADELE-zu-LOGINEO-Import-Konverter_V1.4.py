...
config_gruppe_laa_lehramt = config_xmlsoup.find('gruppe_laa_lehramt').string # group LAA_LEHRAMT
config_gruppe_laa_lehramt_jg = config_xmlsoup.find('gruppe_laa_lehramt_jg').string # group LAA_LEHRAMT_JAHRGANG
...
# create dataframes
data = {"Quell-Id":[], "Nachname":[], "Vorname":[],  "Typ":[], "Seminar":[], "Lehramt":[],  "Jahrgang":[]}
    ...
    if (df1.iloc[i]['Identnummer']) != '':
        ...
        if config_gruppe_laa_lehramt == 'ja':
          if df1.iloc[i]['Lehramt'] in lehraemter:
              data['Seminar'].append('Seminar_'+str(lehraemter[df1.iloc[i]['Lehramt']]))
          else:
              data['Seminar'].append('')
        ...
        if config_gruppe_laa_lehramt_jg == 'ja':
            if df1.iloc[i]['Lehramt'] in lehraemter and df1.iloc[i]['VD1_von'] != '':
              data['Jahrgang'].append('LAA_'+str(lehraemter[df1.iloc[i]['Lehramt']])+'_'+str(df1.iloc[i]['VD1_von'][-4:]+'-'+str(df1.iloc[i]['VD1_von'][-7:-5])))
            else:
...
df2 = pd.DataFrame(data, columns = ['Quell-Id', 'Nachname', 'Vorname', 'Typ', 'Seminar', 'Lehramt', 'Jahrgang'])
